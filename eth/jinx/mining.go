...
// Prepare prepares the Jinx chain for processing a new block at the given height.
func (pl *Jinx) Prepare(ctx context.Context, number uint64) {
	pl.blockchain.Prepare(ctx, number)
}

// ProcessTransaction processes the given transaction and returns the receipt.
func (pl *Jinx) ProcessTransaction(ctx context.Context, tx *types.Transaction) (*core.ExecutionResult, error) {
	return pl.blockchain.ProcessTransaction(ctx, tx)
}

// Finalize finalizes the current block.
func (pl *Jinx) Finalize(ctx context.Context) error {
