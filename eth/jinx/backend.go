...
import (
	...
	"pkg.berachain.dev/jinx/eth/common/hexutil"
	...
	jinxapi "pkg.berachain.dev/jinx/eth/jinx/api"
	"pkg.berachain.dev/jinx/eth/version"
	"pkg.berachain.dev/jinx/lib/utils"
)

// Backend represents the backend object for a Jinx chain. It extends the standard
...
// chain and thus it is not implemented in Jinx.
...
func (b *backend) HeaderByNumber(_ context.Context, number rpc.BlockNumber) (*types.Header, error) {
	...
	case rpc.PendingBlockNumber:
		...
		header := b.jinx.blockchain.CurrentHeader()
		return header, nil
	case rpc.LatestBlockNumber:
		return b.jinx.blockchain.CurrentHeader(), nil
	case rpc.FinalizedBlockNumber:
		block := b.jinx.blockchain.CurrentFinalBlock()
		if block != nil {
			return block, nil
		}
		return nil, errors.New("finalized block not found")
	case rpc.SafeBlockNumber:
		block := b.jinx.blockchain.CurrentSafeBlock()
		if block != nil {
			return block, nil
		}
		return nil, errors.New("safe block not found")
	case rpc.EarliestBlockNumber:
		return b.jinx.blockchain.GetHeaderByNumber(0), nil
	default:
		return b.jinx.blockchain.GetHeaderByNumber(uint64(number)), nil
	}
}
...
func (b *backend) HeaderByHash(_ context.Context, hash common.Hash) (*types.Header, error) {
	return b.jinx.blockchain.GetHeaderByHash(hash), nil
}
...
func (b *backend) BlockByNumber(_ context.Context, number rpc.BlockNumber) (*types.Block, error) {
	...
	case rpc.PendingBlockNumber:
		...
		header := b.jinx.blockchain.CurrentBlock()
		return b.jinx.blockchain.GetBlock(header.Hash(), header.Number.Uint64()), nil
	...
	case rpc.FinalizedBlockNumber:
		header := b.jinx.blockchain.CurrentFinalBlock()
		return b.jinx.blockchain.GetBlock(header.Hash(), header.Number.Uint64()), nil

	case rpc.SafeBlockNumber:
		header := b.jinx.blockchain.CurrentSafeBlock()
		return b.jinx.blockchain.GetBlock(header.Hash(), header.Number.Uint64()), nil

	case rpc.EarliestBlockNumber:
		return b.jinx.blockchain.GetBlockByNumber(0), nil
	}
	// safe to assume number > 0
	return b.jinx.blockchain.GetBlockByNumber(uint64(number)), nil
}

// BlockByHash returns the block with the given `hash`.
func (b *backend) BlockByHash(_ context.Context, hash common.Hash) (*types.Block, error) {
	block := b.jinx.blockchain.GetBlockByHash(hash)
...
func (b *backend) BlockByNumberOrHash(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (*types.Block, error) {
	...
	if hash, ok := blockNrOrHash.Hash(); ok {
		...
		// if blockNrOrHash.RequireCanonical && b.jinx.blockchain.GetCanonicalHash(header.Number.Uint64()) != hash {
		// 	return nil, errors.New("hash is not currently canonical")
		// }
		// block := b.jinx.blockchain.GetBlock(hash, header.Number.Uint64())
		// if block == nil {
...
func (b *backend) StateAndHeaderByNumber(
	...
	state, err := b.jinx.blockchain.StateAtBlockNumber(header.Number.Uint64())
	if err != nil {
...
func (b *backend) GetTransaction(
	...
	txLookup := b.jinx.blockchain.GetTransactionLookup(txHash)
	if txLookup == nil {
...
// PendingBlockAndReceipts returns the pending block (equivalent to current block in Jinx)
// and associated receipts.
func (b *backend) PendingBlockAndReceipts() (*types.Block, types.Receipts) {
	block, receipts := b.jinx.blockchain.PendingBlockAndReceipts()
...
func (b *backend) GetReceipts(_ context.Context, hash common.Hash) (types.Receipts, error) {
	return b.jinx.blockchain.GetReceiptsByHash(hash), nil
}
...
func (b *backend) GetLogs(
	...
	receipts := b.jinx.blockchain.GetReceiptsByHash(blockHash)
...
func (b *backend) GetTd(_ context.Context, hash common.Hash) *big.Int {
	if header := b.jinx.blockchain.GetHeaderByHash(hash); header != nil {
		return b.jinx.blockchain.GetTd(hash, header.Number.Uint64())
	}
	return nil
}
...
func (b *backend) GetEVM(ctx context.Context, msg *core.Message, state vm.GethStateDB,
	...
	if vmConfig == nil {
		...
		vmConfig = b.jinx.blockchain.GetVMConfig()
	}
	txContext := core.NewEVMTxContext(msg)
	return b.jinx.blockchain.GetEVM(ctx, txContext,
		utils.MustGetAs[vm.JinxStateDB](state), header, vmConfig), state.Error
}
...
func (b *backend) GetBlockContext(
	...
	return b.jinx.blockchain.NewEVMBlockContext(header)
}

func (b *backend) SubscribeChainEvent(ch chan<- core.ChainEvent) event.Subscription {
	b.logger.Debug("called eth.rpc.backend.SubscribeChainEvent", "ch", ch)
	return b.jinx.blockchain.SubscribeChainEvent(ch)
}
...
func (b *backend) SubscribeChainHeadEvent(ch chan<- core.ChainHeadEvent) event.Subscription {
	...
	return b.jinx.blockchain.SubscribeChainHeadEvent(ch)
}
...
func (b *backend) SubscribeChainSideEvent(ch chan<- core.ChainSideEvent) event.Subscription {
	...
	return b.jinx.blockchain.SubscribeChainSideEvent(ch)
}
...
func (b *backend) TxPoolContent() (
	...
	pending, queued := b.jinx.blockchain.GetPoolContent()
...
func (b *backend) TxPoolContentFrom(addr common.Address) (
	...
	pending, queued := b.jinx.blockchain.GetPoolContentFrom(addr)
...
func (b *backend) SubscribeNewTxsEvent(ch chan<- core.NewTxsEvent) event.Subscription {
	return b.jinx.blockchain.SubscribeNewTxsEvent(ch)
}
...
func (b *backend) SubscribeRemovedLogsEvent(ch chan<- core.RemovedLogsEvent) event.Subscription {
	...
	return b.jinx.blockchain.SubscribeRemovedLogsEvent(ch)
}
...
func (b *backend) SubscribeLogsEvent(ch chan<- []*types.Log) event.Subscription {
	...
	return b.jinx.blockchain.SubscribeLogsEvent(ch)
}

func (b *backend) SubscribePendingLogsEvent(ch chan<- []*types.Log) event.Subscription {
	return b.jinx.blockchain.SubscribePendingLogsEvent(ch)
}
...
func (b *backend) ClientVersion() string {
	return version.ClientName("jinx-geth")
}
