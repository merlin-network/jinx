...
jinxd config set client chain-id "$CHAINID" --home "$HOMEDIR"
...
for KEY in "${KEYS[@]}"; do
    jinxd genesis add-genesis-account $KEY 100000000000000000000000000ablack --keyring-backend $KEYRING --home "$HOMEDIR"
done

# Sign genesis transaction
jinxd genesis gentx ${KEYS[0]} 1000000000000000000000ablack --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
...
# Collect genesis tx
jinxd genesis collect-gentxs --home "$HOMEDIR"

# Run this to ensure everything worked and that the genesis file is setup correctly
jinxd genesis validate-genesis --home "$HOMEDIR"
...
jinxd start --pruning=nothing "$TRACE" --log_level $LOGLEVEL --api.enabled-unsafe-cors --api.enable --api.swagger --minimum-gas-prices=0.0001ablack --home "$HOMEDIR"