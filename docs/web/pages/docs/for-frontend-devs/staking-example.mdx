  ...
  return (
    <WagmiConfig client={client}>
      <RainbowKitProvider chains={chains}>
        <NextHead>
          <title>Jinx Ethereum Example Dapp</title>
        </NextHead>
...
In order to point this dapp towards a Jinx Ethereum local node,
...
// Configure chain information for local Jinx Ethereum chain
const jinxChain: Chain = {
  id: 2061,
  name: 'Jinx Ethereum's,
  network: 'jinx',
  nativeCurrency: {
    decimals: 18,
    name: 'Jinx Ethereum's,
...
// Configure Wagmi client with Jinx Ethereum chain
...
We will need to add some utility functions to our dApp. These functions will help us interact with the Jinx Ethereum precompile.
...
function Page() {
  return (
    ...
    >
      ...
      <Text fontSize={"4xl"}>Jinx Ethereum Staking Precompile Example</Text>
      <Text fontSize={"xl"}>
        Ensure you have a Local{" "}
        <Link color={"#1da1f1"} href="TODO ADD LINK">
          Jinx Ethereum Node running.
        </Link>
      </Text>
      <Staking />
      <Text fontSize={"xs"}>
        To learn more about Jinx Ethereum please reference our{" "}
        <Link color={"#1da1f1"} href="TODO ADD LINK">
          docs
        </Link>
      </Text>
...
// Default Address of the Staking Precompile Contract on Jinx.
...
Prior to Jinx Ethereum's precompiled contracts, the UX of this task would be more complicated and foreign as it involves
...
You can view this example and others like it on our examples repository on Github [here](https://github.com/berachain/jinx-examples)
